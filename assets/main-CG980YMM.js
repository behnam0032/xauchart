import{r as U,i as C}from"./header-dgUtNWkJ.js";import{H as r,f as E}from"./api-BF5_4Y-g.js";import{g as I,i as M}from"./calculator-BNxaeBxc.js";const y=[{id:"sydney",label:"Sydney",open:22,close:7,color:"#22c55e",pairs:["AUD","NZD"]},{id:"tokyo",label:"Tokyo",open:0,close:9,color:"#3b82f6",pairs:["JPY","AUD"]},{id:"london",label:"London",open:8,close:17,color:"#f97316",pairs:["GBP","EUR"]},{id:"newyork",label:"New York",open:13,close:22,color:"#ef4444",pairs:["USD","CAD"]}];function v(s,e){if(!k())return!1;const{open:n,close:a}=s;return n<a?e>=n&&e<a:e>=n||e<a}function B(s,e,n){if(!v(s,e))return 0;const{open:a,close:i}=s,o=a<i?(i-a)*60:(24-a+i)*60;let t=a<i||e>=a?(e-a)*60+n:(24-a+e)*60+n;return Math.min(100,Math.round(t/o*100))}function w(s,e,n){const{close:a}=s;let i=a*60,o=e*60+n,t=i-o;return t<=0&&(t+=24*60),t}function b(s){const e=Math.floor(s/60),n=s%60;return e===0?`${n}m`:`${e}h ${n}m`}function k(s=new Date){const e=s.getUTCDay(),n=s.getUTCHours();return!(e===6||e===0&&n<22||e===5&&n>=22)}function x(s=new Date){const e=n=>String(n).padStart(2,"0");return{h:e(s.getUTCHours()),m:e(s.getUTCMinutes()),s:e(s.getUTCSeconds()),day:s.getUTCDay(),date:s.getUTCDate(),month:s.getUTCMonth(),year:s.getUTCFullYear(),raw:s}}const D=["یکشنبه","دوشنبه","سه‌شنبه","چهارشنبه","پنجشنبه","جمعه","شنبه"],S=["ژانویه","فوریه","مارس","آوریل","مه","ژوئن","ژوئیه","اوت","سپتامبر","اکتبر","نوامبر","دسامبر"];function L(){function s(){const e=x(),n=parseInt(e.h),a=parseInt(e.m),i=document.getElementById("clock"),o=document.getElementById("clockDate");i&&(i.textContent=`${e.h}:${e.m}:${e.s}`),o&&(o.textContent=`${D[e.day]} ${e.date} ${S[e.month]} ${e.year} — UTC`);const t=k(e.raw),d=document.getElementById("mkBadge");d&&(d.className="market-badge "+(t?"open":"closed"));const p=document.getElementById("mkText");p&&(p.textContent=t?"بازار باز است":"بازار بسته است");const u=document.getElementById("weekendBanner");u&&u.classList.toggle("show",!t);const g=document.getElementById("sessionPills");g&&(g.innerHTML=y.map(l=>{const c=v(l,n),m=c?b(w(l,n,a)):"";return`<div class="session-pill ${c?"on":"off"}">
          <span class="sname">${l.label}</span>
          <span class="shours">${l.open}:00–${l.close}:00</span>
          ${c?`<span class="sleft">${m} مانده</span>`:""}
        </div>`}).join(""));const h=document.getElementById("sessRows");h&&(h.innerHTML=y.map(l=>{const c=v(l,n),m=B(l,n,a),T=c?b(w(l,n,a)):"",$=c?"var(--up)":"var(--down)";return`<div class="sess-row">
          <span class="sess-name">${l.label}</span>
          <div class="sess-track"><div class="sess-fill" style="width:${c?m:0}%;background:${$}"></div></div>
          <span class="sess-time">${l.open}:00–${l.close}:00</span>
          <span class="sess-tag ${c?"on":"off"}">${c?"باز · "+T:"بسته"}</span>
        </div>`}).join(""))}setInterval(s,1e3),s()}const P=s=>new Intl.NumberFormat("fa-IR").format(Math.round(s)),N=(s,e=2)=>new Intl.NumberFormat("en-US",{minimumFractionDigits:e,maximumFractionDigits:e}).format(s),F=(s,e)=>e==="دلار"?N(s,s<10?4:2):P(s);document.getElementById("app").innerHTML=U("home")+`
<main class="main">
  <div class="clock-bar">
